...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, noise=False):
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        self_output, layer_q, layer_k, layer_v = self.self(input_tensor, attention_mask)
        attention_output = self.output(self_output, input_tensor)
        return attention_output, layer_q, layer_k, layer_v
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        attention_output, layer_q, layer_k, layer_v = \
            self.attention(hidden_states, attention_mask)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)

        return layer_output, layer_q, layer_k, layer_v
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        for _, layer_module in enumerate(self.layer):
            ...
            hidden_states, layer_q, layer_k, layer_v = \
                layer_module(hidden_states, attention_mask)
            all_encoder_qs.append(layer_q)
            all_encoder_ks.append(layer_k)
            all_encoder_vs.append(layer_v)
            
        all_encoder_layers.append(hidden_states)
        return all_encoder_layers, all_encoder_qs, all_encoder_ks, all_encoder_vs
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        encoded_layers, layer_qs, layer_ks, layer_vs = \
        ...
        return encoded_layers, layer_qs, layer_ks, layer_vs, pooled_output
    ...
    def get_bert_output(self, embedding_output, attention_mask, 
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, infer=False):
        bert_outputs = self.bert(input_ids, token_type_ids, attention_mask,
                                 output_all_encoded_layers=True)
        prediction_scores = self.cls(bert_outputs[0][-1])
...
class TinyBertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                labels=None, mixup=False, lam=None, is_student=False):
        if mixup:
            return self.impl_mixup(input_ids, token_type_ids, attention_mask, labels,
                                   lam, is_student)
        else:
            return self.impl_forward(input_ids, token_type_ids, attention_mask, labels,
                                     is_student)
    
    def impl_forward(self, input_ids, token_type_ids, attention_mask, labels, is_student):
        
        sequence_output, q_output, k_output, v_output, pooled_output = \
        ...
        if is_student:
            for s_id, sequence_layer in enumerate(sequence_output):
                tmp.append(self.fit_dense(sequence_layer))
            sequence_output = tmp
        return logits, q_output, k_output, v_output, sequence_output
    
    def impl_mixup(self, input_ids, token_type_ids, attention_mask, labels, lam, is_student):
        ...
        embedding_output = lam * embedding_output + (1 - lam) * embedding_output[rev]
        ...
        attention_mask = torch.where(attention_mask1 + attention_mask2 == 0, 
                                     torch.zeros(attention_mask1.size()).to(device), 
                                     torch.ones(attention_mask2.size()).to(device))
        sequence_output, q_output, k_output, v_output, pooled_output = \
            self.bert.get_bert_output(embedding_output, attention_mask=attention_mask,
                                      output_all_encoded_layers=True)
        logits = self.classifier(torch.relu(pooled_output))
        tmp = []
        if is_student:
